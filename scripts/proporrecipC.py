...
# my classes
from proporclasses import MyClass2, Tagger, MyPatts2, MyClassStats2
...
class ProporRamseyAll:
    ...
    def occStats(self,path,format,list,plotting):
        ...
        # plotting vars
        fignameCum = "Base and Ramsey GQs"
    ...
    def fileStatsCum(self,path,fileids):
        ...
        # starting the title
        tit = "Base and Ramsey GQs"

        # stat classes
        C1 = MyClassStats2("ari",[],0,tit)
        C2 = MyClassStats2("cnt",[],0,tit)
        C3 = MyClassStats2("pro",[],0,tit)
        C4 = MyClassStats2("ari+\nrecip",[],0,tit)
        C5 = MyClassStats2("cnt+\nrecip",[],0,tit)
        C6 = MyClassStats2("pro+\nrecip",[],0,tit)

        self.classstatsCum = [C1,C2,C3,C4,C5,C6]
        ...
        for idf in fileids:
            ...
            # class 4
            P4 = MyPatts2(aristotelian).P
            N4 = MyPatts2(eachother).P
            #-------------------------------------------------------------------
            c4 = MyClass2(P4,N4,idf,0,0,"ari+\nrecip",my_tagger)
            print "---------------------------------------------------"
            print " Computing aristotelian + recip"
            print "---------------------------------------------------"
            c4.openFile2(path+"/"+idf,c4.pats,c4.patts)
            print "==> aristotelian + recip : " + `c4.count` + " matches"

            ####################################################################

            # class 5
            P5 = MyPatts2(counting).P
            N5 = MyPatts2(eachother).P
            #-------------------------------------------------------------------
            c5 = MyClass2(P5,N5,idf,0,0,"cnt+\nrecip",my_tagger)
            print "---------------------------------------------------"
            print " Computing counting + recip"
            print "---------------------------------------------------"
            c5.openFile2(path+"/"+idf,c5.pats,c5.patts)
            print "==> counting + recip : " + `c5.count` + " matches"

            ####################################################################

            # class 6
            P6 = MyPatts2(proportional).P
            N6 = MyPatts2(eachother).P
            #-------------------------------------------------------------------
            c6 = MyClass2(P6,N6,idf,0,0,"pro+\nrecip",my_tagger)
            print "---------------------------------------------------"
            print " Computing proportional + recip"
            print "---------------------------------------------------"
            c6.openFile2(path+"/"+idf,c6.pats,c6.patts)
            print "==> proportional + recip : " + `c3.count` + " matches"

            ####################################################################
            ####################################################################

            # total cum count
            tot = (c1.count + c2.count + c3.count
                   + c4.count + c5.count + c6.count) + 1
            print "==================================================="
            ...
            filestatsCum = [c1,c2,c3,c4,c5,c6]
